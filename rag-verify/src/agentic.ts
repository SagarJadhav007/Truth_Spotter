...
class AgenticRAGVerifier {
  ...
  private createGenerateQueriesTool() {
    return new DynamicStructuredTool({
      ...
      schema: z.object({
        claimAnalysis: z.string().describe('The claim analysis output to base queries on'),
      }),
...
- Return ONLY the 3 queries, one per line, no bullets or extra text`;
          ...
          const queries = (result.content as string)
            .split('\n')
            .map(q => q.trim())
            .filter(Boolean)
            .slice(0, 3);
  ...
  private createFetchNewsTool() {
    return new DynamicStructuredTool({
      ...
      schema: z.object({
        queries: z.array(z.string()).describe('Array of search queries to fetch news for'),
      }),
      func: async ({ queries }) => {
        try {
  ...
  private createRetrieveEvidenceTool() {
    return new DynamicStructuredTool({
      ...
      schema: z.object({
        claim: z.string().describe('The claim to find relevant evidence for'),
      ...
      func: async ({ claim, limit = 15 }) => {
        try {
          ...
          const snippets = docs.slice(0, 6).map((doc, idx) => ({
            index: idx + 1,
            source: (doc.metadata as any)?.source || (doc.metadata as any)?.link || 'unknown',
  ...
  private createFactCheckTool() {
    return new DynamicStructuredTool({
      ...
      schema: z.object({
        ...
        evidenceSnippets: z.string().describe('JSON string of evidence snippets'),
      }),
      func: async ({ claim, evidenceSnippets }) => {
        try {
          ...
          const snippetText = snippets.map((s: any) => 
            `[#${s.index}] Source: ${s.source} (${s.date})\n${s.snippet}...`
          ).join('\n\n');
...
REASONING: <2–4 sentences explaining why, referencing snippet indices like [#1], [#2]>`;
  ...
  // ==============================
  private async invokeLLMWithTools(
    systemPrompt: string,
    userMessage: string,
    ...
    while (iterationCount < maxIterations) {
      ...
      const toolDescriptions = tools.map(t => 
        `Tool: ${t.name}\nDescription: ${t.description}\nParameters: ${JSON.stringify(t.schema.shape)}`
      ).join('\n\n');
...
FINAL_ANSWER: your response here`;
        ...
        if (toolMatch) {
          ...
          if (tool && paramsMatch) {
            try {
              ...
              messages.push(
                { role: 'assistant', content },
                { role: 'user', content: `Tool Error: ${error}` }
              );
              continue;
            }
          }
  ...
  // ==============================
  private async runClaimAnalystAgent(claim: string): Promise<string> {
...
Keep it structured and concise.`;
    ...
    try {
      const result = await this.invokeLLMWithTools(
        systemPrompt,
  ...
  // ==============================
  private async runEvidenceResearcherAgent(
    claim: string,
...
Provide a summary of what you found.`;
    ...
    try {
      await this.invokeLLMWithTools(
        systemPrompt,
        `Research evidence for this claim: "${claim}"\n\nClaim Analysis:\n${analystOutput}`,
        tools,
        6
      );
...
- KB Docs (vector hits): ${this.toolContext.kbDocs.length}`;
  ...
  // ==============================
  private async runFactCheckerAgent(
    claim: string,
    analystOutput: string,
...
The tool will return SUPPORTED, REFUTED, or INCONCLUSIVE with reasoning.`;
    ...
    try {
      const result = await this.invokeLLMWithTools(
        systemPrompt,
        `Fact-check this claim: "${claim}"\n\nPass the evidence snippets to the fact_check tool.`,
        tools,
        3
      );
...
- INCONCLUSIVE: isVerified=false, confidence=60 or lower`;
  ...
  private mapEvidence(docs: Document[]): NewsArticle[] {
    return docs.slice(0, 8).map(doc => ({
      title: this.safeField((doc.metadata as any)?.title ?? (doc.metadata as any)?.source ?? 'Untitled'),
  ...
  async verifyClaimAgentic(claim: string): Promise<AgenticVerificationResult> {
    ...
    const factCheckerOutput = await this.runFactCheckerAgent(
      claim,
      analystOutput,
      evidenceContext
    );
